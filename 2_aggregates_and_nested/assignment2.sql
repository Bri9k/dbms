...
ON s.ID = t.ID AND (t.course_id, t.sec_id, t.semester, t.year) = (ch.course_id, ch.sec_id, ch.semester, ch.year) AND ch.ID = i.ID
WHERE i.name = "Srinivasan"
GROUP BY s.ID
...
---- 10 Find the names of all students whose advisor has taught the maximum number of courses (multiple offerings of a course count as only 1). (Note: this is a complex query, break it into parts by using the with clause.)
